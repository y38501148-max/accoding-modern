...
班级面板提供「代码复核」，提交页提供复核侧栏。整场批次支持全量规则、候选复核和未命中分层抽样，面板仅显示当前班级中模型标记为需要复核或优先复核的有效结果，正常结果不进入列表与导出。新批次默认同人同题相邻两次提交成对抽样，最多 300 对、600 份代码。填写共享只读令牌后，可以查询已采集提交的规则事实、源码与前一版本差异；支持本机人工记录和导出。读取与刷新不会创建模型任务。详见 [代码复核使用说明](docs/ai-review.md)。
