...
班级面板提供「代码复核」，提交页提供复核侧栏。整场批次支持全量规则、候选复核和未命中分层抽样，面板按当前班级显示覆盖数量及真实处理状态。填写共享只读令牌后，可以查询已采集提交的规则事实、源码与前一版本差异；支持本机人工记录和导出。读取与刷新不会创建模型任务。详见 [代码复核使用说明](docs/ai-review.md)。
